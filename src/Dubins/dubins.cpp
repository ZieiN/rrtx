...
double Dubins::distance(const Point *state1, const Point *state2) const {
    if (isSymmetric_)
        return rho_ * std::min(dubins(state1, state2).length(), dubins(state2, state1).length());
    return rho_ * dubins(state1, state2).length();
}

void Dubins::interpolate(const Point *from, const Point *to, double t, Point *point) const {
    bool firstTime = true;
    DubinsPath path;
    interpolate(from, to, t, firstTime, path, point);
}

void Dubins::interpolate(const Point *from, const Point *to, double t, bool &firstTime, DubinsPath &path,
                         Point *point) const {
    {
        if (firstTime)
        {
...
void Dubins::copyState(Point *to, const Point *from) const {
    to->x_ = from->x_;
    to->y_ = from->y_;
